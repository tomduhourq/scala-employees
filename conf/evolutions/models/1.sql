...
alter table `EMPLOYEES` add constraint `POS_FK` foreign key(`POSITION`) references `POSITIONS`(`ID`) on update NO ACTION on delete CASCADE;
alter table `POSITIONS` add constraint `COM_FK` foreign key(`COMPANY`) references `COMPANIES`(`ID`) on update NO ACTION on delete CASCADE;
